...
export default function StaffMedpathElitePage() {
  ...
  const filtered = eliteStudents.filter((s) => {
  ...
  return (
    <div style={{ padding: '32px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px', marginBottom: '24px' }}>
        {[
          { label: 'Total Elite Students', value: eliteStudents.length, color: '#EA580C', bg: '#FFF7ED' },
          { label: 'In Progress', value: eliteStudents.filter(s => Object.values(s.stages).some(v => v === 'in_progress')).length, color: '#2563EB', bg: '#EFF6FF' },
          { label: 'Fully Completed', value: eliteStudents.filter(s => Object.values(s.stages).every(v => v === 'completed')).length, color: '#16A34A', bg: '#F0FFF4' },
        ].map((stat, i) => (
